...
export default class LevelHealth {
  ...
  public static decryptMetaShare = (
    ...
    try {
      ...
      const { shareId, encryptedSecret, encryptedShare } = decryptedMetaShare
      const generatedShareId = crypto
        .createHash( 'sha256' )
        .update( JSON.stringify( encryptedSecret ? encryptedSecret : encryptedShare.pmShare ) )
        .digest( 'hex' )
  ...
  public generateLevel2Shares = ( secondaryMnemonics:string, answer: string ): {
    ...
    if( !PrimaryMnemonics ){
      ...
      for ( let i = 0; i < this.metaSharesKeeper.length; i++ ) {
        ...
        decryptedShareArr.push( element.encryptedSecret ? element.encryptedSecret : element.encryptedShare.pmShare )
      }
    ...
    if( !secondaryMnemonics ){
      ...
      for ( let i = 0; i < secondaryShares.length; i++ ) {
        ...
        decryptedShareArr.push( element.encryptedSecret ? element.encryptedSecret : element.encryptedShare.smShare )
      }
      const { decryptedSecrets } = LevelHealth.decryptSecrets( decryptedShareArr, answer )
